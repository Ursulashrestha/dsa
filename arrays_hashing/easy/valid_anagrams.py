...
def isAnagaram(self,s:str,t:str)-> bool:
    ...
    count = [0]* 26 #frequency array of size 26
    #ord('a')= 97
    for i in range(len(s)):
        count[ord(s[i])-ord('a')]+=1 #incrementing counts for characters in the first string
        count[ord(t[i])-ord('a')]-=1 # decrementing for the second. If the strings are anagrams, all counts cancel out and remain zero.
